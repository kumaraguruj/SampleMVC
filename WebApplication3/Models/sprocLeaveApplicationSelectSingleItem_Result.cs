//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace WebApplication3.Models
{
    using System;
    
    public partial class sprocLeaveApplicationSelectSingleItem_Result
    {
        public string cCardNumber { get; set; }
        public Nullable<System.DateTime> dtFrom { get; set; }
        public Nullable<System.DateTime> dtTo { get; set; }
        public Nullable<System.DateTime> dtHalfDate { get; set; }
        public string cLeaveTypeCode { get; set; }
        public Nullable<decimal> nDays { get; set; }
        public string cFullHalf { get; set; }
        public string cStatus { get; set; }
        public string cReason { get; set; }
        public string cInclude { get; set; }
        public string cComments { get; set; }
        public string cEmailIDs { get; set; }
        public string cemployeename { get; set; }
        public string cemployeecode { get; set; }
    }
}
